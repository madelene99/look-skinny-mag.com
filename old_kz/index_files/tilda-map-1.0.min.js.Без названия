...
var script=document.createElement("script");script.type="text/javascript";script.src="//maps.google.com/maps/api/js?key="+jQuery.trim(key)+"&callback=t_handleGoogleApiReady_"+grecid+addParams;document.body.appendChild(script);window.googleapiiscalled=!0}else{setTimeout(function(){t_appendGoogleMap(grecid,key)},200)}}}
...
if(arMarkers.length>1){map.fitBounds(bounds);if(map.getZoom()>parseInt(el.attr('data-map-zoom'))||map.getZoom()==0){map.setZoom(parseInt(el.attr('data-map-zoom')))}}
google.maps.event.addDomListener(window,"resize",function(){var center=map.getCenter();var zoom=parseInt(el.attr('data-map-zoom'));google.maps.event.trigger(map,"resize");map.setCenter(center);if(arMarkers.length>1){map.fitBounds(bounds);if(zoom>0&&(map.getZoom()>zoom||map.getZoom()==0)){map.setZoom(zoom)}}})})}
function t_appendYandexMap(recid,key){var yarecid=recid;if(typeof ymaps==='object'&&typeof ymaps.Map==='function'){t_handleYandexApiReady(recid)}else{if(window.yandexmapsapiiscalled!==!0){var runfunc='window.t_handleYandexApiReady_'+yarecid+' = function () { return t_handleYandexApiReady("'+yarecid+'") }';eval(runfunc);var script=document.createElement("script");script.type="text/javascript";script.src="https://api-maps.yandex.ru/2.1/?lang=ru-RU&coordorder=latlong&onload=t_handleYandexApiReady_"+yarecid;if(key>''){script.src=script.src+'&apikey='+key}
...
if(zoom>0&&(map.getZoom()==0||map.getZoom()>zoom)){map.setZoom(zoom)}});if($isMobile){var lastTap=0;$('#rec'+recid).mousedown(function(){var now=(new Date()).valueOf();var diff=(now-lastTap);lastTap=now;if(diff<250){if(window.isDragMap){window.isDragMap=!1}else{window.isDragMap=!0}
